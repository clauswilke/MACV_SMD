...
\caption{\label{fig:rmsf}\title{RMSF values during equilibration.} The RMSF values for every site in the bound complex computed during the equilibration phase of the protocol. Each color represents the average over 20 trajectories of a single mutant. Indices 17-25 are the hTfR flexible loop. The plot shows the flexibility of each site is essentially independent of mutation, and two sites above 0.72~\AA\ are found on the receptor in the interface.}
\end{figure}
