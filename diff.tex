...
\DIFadd{Again, the NAMD configuration file is made available via github. We used a similar configuration to that in equilibration. One significant difference was to make a cubic water box with a side length equal to the long axis of the complex plus a 10~\AA}\  \DIFadd{buffer on either side, and simply restrict center of mass motion with the NAMD setting. This was done to avoid affecting the system energy while calculating free energy differences. 
}
...
The statistical package R was used for all further analysis and visualization. Each of the 50 independent trajectories per mutant produced a fairly noisy force curve. \DIFdelbegin \DIFdel{To reduce the noise, we averaged each point with }\DIFdelend \DIFaddbegin \DIFadd{The force curves for each mutant were smoothed either by averaging over many replicates as in Figure~\ref{fig:force_curve} or by selecting }\DIFaddend a 50 \DIFdelbegin \DIFdel{point interpolation window }\DIFdelend \DIFaddbegin \DIFadd{frame interpolation window for averaging. The latter approach was used for statistical analysis}\DIFaddend . The \DIFdelbegin \DIFdel{window ensured that the averaged maximum applied force was never cut off near the beginning of the simulation due to lost time steps (caused by averaging). The }\DIFdelend two primary descriptive statistics we used were maximum interpolated applied force and total area under the interpolated curve (AUC). We used the pairwise.t.test function with the FDR p-value correction to test for significance. The ggplot \citep{ggplot} package was used to generate most of the figures. 
...
\caption{\label{fig:fep}\title{\DIFaddFL{Max force versus free energy perturbation.}} \DIFaddFL{Scatter plot of maximum force in SMD versus the relative free energy difference calculated by FEP for all 10 mutants tested plus the WT complex. The WT complex for FEP was simply set to 0.0. The correlation between the two is $r=-0.767$ with $p=0.005857$.}}
\end{figure}
