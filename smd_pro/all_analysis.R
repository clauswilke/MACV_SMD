...
scatter.plot <- function(df, name) {
  ...
  g <- ggplot()
  g <- g + geom_point(data=df, aes(x=distance, y=force, color=id), size=2.5) + scale_colour_manual(values=c("black", "#FF9999"))
  g <- g + theme(strip.background=element_blank())
  g <- g + ylab("Force (pN)")
  g <- g + xlab("Distance (A)")
  g <- g + scale_y_continuous(breaks=seq(-150, 1500, 150), limits=c(-15, 750))
  g <- g + scale_x_continuous(breaks=seq(0, 12, 2), limits=c(0, 10))
  ...
  ggsave(g, file=paste("~/Desktop/MACV_SMD/figures/",name,sep=''), width=7, height=5)
}
...
diff.plot <- function(diff.df) {
  ...
  ggsave(g, file='~/Desktop/MACV_SMD/figures/sensitivity_histogram.pdf', width=7, height=5)
}
...
split.distance.wt <- data.frame(split(wt$distance, wt$id))
split.distance.ya <- data.frame(split(ya$distance, ya$id))
split.distance.yd <- data.frame(split(yd$distance, yd$id))
split.distance.yt <- data.frame(split(yt$distance, yt$id))
split.distance.na <- data.frame(split(na$distance, na$id))
split.distance.nw <- data.frame(split(nw$distance, nw$id))
split.distance.nk <- data.frame(split(nk$distance, nk$id))
split.distance.ra <- data.frame(split(ra$distance, ra$id))
split.distance.na.ya <- data.frame(split(na.ya$distance, na.ya$id))
split.distance.nw.ya <- data.frame(split(nw.ya$distance, nw.ya$id))
split.distance.ra.ya <- data.frame(split(ra.ya$distance, ra.ya$id))
...
print(pairwise.t.test(scale(df$y1), df$id, p.adjust.method="fdr"))

#fit <- aov(y1 ~ id, data = df)
#print(TukeyHSD(fit))

ddply(df, .(id), colwise(mean))
ddply(df, .(id), colwise(sd))

scatter.plot(data.frame(id=rep('1',length(rowMeans(split.distance.wt))), distance=rowMeans(split.distance.wt), force=rowMeans(split.force.wt)), "force_curves/wt.pdf")
scatter.plot(data.frame(id=rep('1',length(rowMeans(split.distance.ya))), distance=rowMeans(split.distance.ya), force=rowMeans(split.force.ya)), "force_curves/ya.pdf")
scatter.plot(data.frame(id=rep('1',length(rowMeans(split.distance.yd))), distance=rowMeans(split.distance.yd), force=rowMeans(split.force.yd)), "force_curves/yd.pdf")
scatter.plot(data.frame(id=rep('1',length(rowMeans(split.distance.yt))), distance=rowMeans(split.distance.yt), force=rowMeans(split.force.yt)), "force_curves/yt.pdf")
scatter.plot(data.frame(id=rep('1',length(rowMeans(split.distance.na))), distance=rowMeans(split.distance.na), force=rowMeans(split.force.na)), "force_curves/na.pdf")
scatter.plot(data.frame(id=rep('1',length(rowMeans(split.distance.nw))), distance=rowMeans(split.distance.nw), force=rowMeans(split.force.nw)), "force_curves/nw.pdf")
scatter.plot(data.frame(id=rep('1',length(rowMeans(split.distance.nk))), distance=rowMeans(split.distance.nk), force=rowMeans(split.force.nk)), "force_curves/nk.pdf")
...
tmp.scatter.combined <- rbind(tmp.scatter.wt, tmp.scatter.ya)
scatter.plot(tmp.scatter.combined, "force_curves/combined.pdf")
...
smd.points <- function(df) {
  ...
  g <- g + scale_y_continuous(breaks=seq(500, 1000, 100), limits=c(550, 850))
  g <- g + theme(panel.border=element_blank(), axis.line=element_line())
  g <- g + theme(axis.title.x = element_text(size=18, vjust=0))
